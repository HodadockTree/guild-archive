    ...
    <main className="app-shell">
      ...
      <div className="space-y-3">
        <Link
          ...
          className="ui-focus-ring inline-flex size-8 items-center justify-center rounded-md text-lg text-[var(--brand-strong)] transition hover:bg-sky-50"
          href="/"
        >
      ...
      <section aria-labelledby="member-summary-title">
        ...
        <dl className="mt-3 grid grid-cols-2 gap-x-4 gap-y-3 border-y border-sky-100 bg-white px-4 py-5 sm:gap-x-6 sm:gap-y-4 lg:grid-cols-5">
          <div className="rounded-md bg-sky-50/70 px-3 py-2"><dt className="text-xs text-slate-500">총 참여 활동</dt><dd className="mt-1 text-xl font-bold text-slate-900">{summary.totalActivityCount}회</dd></div>
          <div><dt className="text-xs text-slate-500">함께한 기간</dt><dd className="mt-1 font-semibold text-slate-900">{summary.togetherDayCount ? `${summary.togetherDayCount}일${member.status === "active" ? "째" : "간"}` : "-"}</dd></div>
          ...
          <div className="col-span-2 lg:col-span-1"><dt className="text-xs text-slate-500">가장 많이 함께한 활동</dt><dd className="mt-1 font-semibold text-slate-900">{mostFrequentLabel}</dd></div>
        </dl>
      </section>
