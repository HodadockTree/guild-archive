...
기존 운영 DB에는 기능 배포 전에 아래의 추가 마이그레이션을 한 번 적용합니다.

```bash
npm run db:archive-metadata:local   # 로컬 기존 DB
npm run db:archive-metadata:remote  # 운영 D1
```

이 마이그레이션은 길드원 테이블에 nullable `gender`, `birthYear`를 추가하고,
활동과 분리된 `monthly_highlights` 테이블 및 월별 조회 인덱스를 생성합니다.
기존 길드원·활동·이미지 데이터는 변경하지 않습니다.
...
기존 LocalStorage 백업에 `imageDataUrl`이 남아 있다면 관리 화면의 데이터 관리 도구에서
JSON 파일을 선택한 뒤 **과거 이미지 사전 점검**을 실행할 수 있습니다.

- 일반 서버 DB 가져오기와 달리 활동·길드원 데이터를 삭제하지 않습니다.
- 활동 ID가 일치하고 D1의 `imageDataUrl`이 비어 있을 때만 이미지를 보충합니다.
- 기존 `imageUrl`과 이미 저장된 `imageDataUrl`은 변경하지 않습니다.
- 개별 이미지는 약 1.5MB, 요청 묶음은 약 4.5MB 이하로 제한합니다.
- 사전 점검 결과를 확인한 뒤에만 실제 보충 버튼이 활성화됩니다.
- 이 기능은 R2 이전을 위한 임시 복구 경로입니다.

## 월별 주요 기록과 관리자용 길드원 정보

- `/admin`의 **월별 주요 기록 관리**에서 게임 업데이트, 게임 이벤트, 길드 소식,
  기타 기록을 월별로 여러 건 관리할 수 있습니다.
- 주요 기록은 활동 기록과 별도 D1 테이블에 저장되며 `/archive`와 `/viewer`에만
  필요한 공개 필드로 표시됩니다.
- 길드원 성별과 출생연도는 선택 입력이며, 두 자리 연도는 현재 연도를 기준으로
  네 자리 연도로 변환합니다.
- 성별, 출생연도, 계산된 연령대와 관리자 메모는 관리자 화면과 보호된 데이터
  가져오기 흐름에서만 취급하고 공개 API 응답에는 포함하지 않습니다.

## 주의사항

- 로그인, 세션/쿠키 인증, 권한 관리는 없습니다. 서버 가져오기와 월별 주요 기록 관리 API는 동일한 단일 관리자 토큰으로 보호됩니다.
- 관리 화면의 LocalStorage 데이터와 D1 서버 DB는 자동 동기화되지 않습니다.
