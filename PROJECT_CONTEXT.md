...
길드원 관리 시스템을 만드는 것이 아니라, 길드 활동 아카이브를 만드는 것이 핵심입니다. 주요 목적은 활동 기록 입력, 길드원별 활동 통계 확인, 과거 기록 보관입니다.

v1.2는 활동 기록 구조와 통계 개선에 집중했고, v1.2.1은 실제 데이터 입력 중 발견한 UX 문제를 보정한 버전입니다.
...
```text
app/
  page.tsx              # 메인 화면, 길드원/활동 입력, 활동 기록 UI
...
v1.2.1은 새 기능을 크게 늘린 버전이라기보다, 실제 데이터 입력 중 발견한 불편함을 줄인 UX 보정 버전입니다.

활동 기록 추가/수정/삭제 후 완료 메시지를 보여주고, 전체 활동 기록에는 최신순/오래된순 정렬과 참여 인원 수 표시가 추가되었습니다.

비공정 입력은 드롭다운 대신 오션헤븐/아우로라 버튼형 입력으로 정리되었습니다. 버튼을 누르면 `airshipType`과 기본 제목이 함께 설정되며, 제목 입력창에서 직접 수정할 수 있습니다.

참여자 검색은 닉네임 일부 검색, 영문/숫자 검색에 더해 한글 초성 검색을 지원합니다. 활동중/탈퇴 길드원 모두 같은 검색 기준을 사용하고, 검색 중에도 선택 상태가 유지되어야 합니다.

길드원 삭제는 활동 기록이 없는 길드원에게만 허용됩니다. 활동 기록이 있는 길드원은 탈퇴 처리로 보존해야 합니다.
